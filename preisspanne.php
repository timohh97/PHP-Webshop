...
<html>
    ...
    <form method="post" action="createNewProduct.php" enctype="multipart/form-data">
        ...
        if($max<$min)
        {
           echo "<script>alert('Der minimale Preis muss <= dem maximalen Preis sein!')</script>";   
               ...
               {
                 $query = "SELECT * FROM shop WHERE category='$category'";
               }
        ...
        if($category=="all" || is_null($category))
        {
           $query = "SELECT * FROM shop WHERE price BETWEEN '$min' AND '$max'";
            
        }
        else
        {
          $query = "SELECT * FROM shop WHERE price BETWEEN '$min' AND '$max' AND category='$category'";
        }
